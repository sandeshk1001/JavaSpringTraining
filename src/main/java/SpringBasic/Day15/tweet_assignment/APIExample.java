...
public class APIExample {
    ...
    -------------------users api----------------------
     ------------------------------------------------*/
    public APIExample() {
        session = Utility.getSessionAnotation(User.class);
    ...
    public ModelAndView getRegiseteredData(@RequestBody MultiValueMap<String, String> formData){
        return errorMessageModelAndView(createUser(formData).toString());
    }

    // create a new user --> POST
    @PostMapping("/user/createUser")
    private ResponseEntity<String> createUser(@RequestBody MultiValueMap<String,String> requestbody){
        allAccDetails();
        ResponseEntity<String> responseEntity=null;
        String name=requestbody.get("name").get(0);
        String email=requestbody.get("email").get(0);
        ...
        if (containsInvalidChar(name))
            ...
            Session session= Utility.getSessionAnotation(User.class);
    ...
    Map<String, User> allAccDetails() {
        ...
        Session session = Utility.getSessionAnotation(User.class);
        List<Object[]> list = session.createQuery("select name,email,password from user", Object[].class).getResultList();
        for (int i = 0; i < list.size(); i++) {
            Object[] arr = list.get(i);
            User user = new User(arr[0].toString(), arr[1].toString(), arr[2].toString());
            userProfileMap.put(arr[1].toString(), user);
        }
        session.close();
    ...
    /**
     * @method POST
     * @use using that we can create tweet
     * @param tweet object
     * @param password
     * @return return response is tweet created or not or any error
     */
